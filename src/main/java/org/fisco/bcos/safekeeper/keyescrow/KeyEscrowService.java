...
public class KeyEscrowService {
     ...
     * add key row.
     */
    public void addKeyRow(String account, PrivateKeyInfo keyInfo) throws SafeKeeperException {
     ...
     * delete key info.
     */
    public void deleteKeyRow(String account, String keyAlias) throws SafeKeeperException {
     ...
     */
    public void accountWithKeyExist(String account, String keyAlias) throws SafeKeeperException {
        if (StringUtils.isBlank(account)) {
            throw new SafeKeeperException(ConstantCode.ACCOUNT_NAME_EMPTY);
        }
        if (StringUtils.isBlank(keyAlias)) {
            throw new SafeKeeperException(ConstantCode.KEY_ALIASES_EMPTY);
        }
     ...
     */
    public void accountWithKeyNotExist(String account, String keyAlias) throws SafeKeeperException {
        if (StringUtils.isBlank(account)) {
            throw new SafeKeeperException(ConstantCode.ACCOUNT_NAME_EMPTY);
        }
        if (StringUtils.isBlank(keyAlias)) {
        ...
        int count = countOfAccountWithKey(account, keyAlias);
        if (count > 0) {
            throw new SafeKeeperException(ConstantCode.KEY_EXISTS);
        }
    }
